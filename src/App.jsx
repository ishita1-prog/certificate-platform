...
function App() {
  ...
  const handleExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      if (unmatched.size > 0) {
        ...
        alert('Error processing Excel file: ' + (error?.message || 'Unknown error. Please check the file format.'));
      }
    };
  ...
  const handleDownload = async (certId) => {
    ...
    try {
      ...
      alert('Could not download certificate. ' + (error?.message || 'Please try again.'));
    }
  };
  ...
  const handleDownloadPDF = async (skipMissingCheck = false) => {
    ...
    try {
      ...
      if (addedPages === 0) {
        alert('No certificates were ready for export. Please try again.');
        return;
      }
      ...
      alert('Could not generate PDF. ' + (error?.message || 'Please try again.'));
    } finally {
      ...
      <div className="flex-1 flex flex-col" style={{ padding: '24px', minHeight: 0 }}>
        ...
        <div
          ...
          style={{
            ...
            minHeight: '600px',
            overflowX: 'hidden',
            outline: dragOverCertificates ? '2px dashed #4736FE' : 'none',
            outlineOffset: dragOverCertificates ? '-4px' : '0',
