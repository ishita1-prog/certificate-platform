...
import { FileSpreadsheet, Download, ChevronDown, ChevronUp, ArrowLeft, Image, Trash2 } from 'lucide-react';
...
function PPTCreator({ onBack }) {
  ...
  // Title/subtitle are fixed copy today; setter kept for future editable UI.
  ...
  const removeBackgroundAPI = async (file) => {
    try {
      ...
      // Use relative /api path — Vite proxies to the backend, so this
      // works over localhost, LAN, and public tunnels alike.
      ...
      setBgRemovalError(error.message);
      return null;
    }
  };
  ...
  const handleMultipleImageUpload = async (e) => {
    ...
    for (const file of files) {
      if (file.type.startsWith('image/')) {
        ...
        setProcessingProgress(`Removing BG ${processedCount + 1}/${totalFiles}: ${awardeeName}`);
                                ...
                                <div key={awardee.id} style={{
                                  ...
                                  <div style={{ position: 'relative', marginBottom: '10px' }}>
                                    <div 
                                      ...
                                      onClick={() => !awardeeImage && handleFrameClick(awardee.name)}
